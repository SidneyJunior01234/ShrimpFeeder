...
1. **Instalar o WSL 2:** Abra o Prompt de Comando ou PowerShell (como Administrador) e execute o seguinte comando:
...
2. **Verificar a Versão (Opcional):** Após a reinicialização, abra o terminal do WSL e confirme a versão:
...
1. **Atualizar o Sistema:** Dentro do seu terminal WSL, atualize a lista de pacotes:

```sudo apt update && sudo apt upgrade -y```


2. **Instalar Python e Venv:** Certifique-se de que o Python 3 e o módulo venv estão instalados:
...
1. **Navegar até o Local:** Navegue até o diretório onde você deseja armazenar o projeto (por exemplo, na pasta de usuário do Linux ou em um diretório montado do Windows, como /mnt/c/Users/SeuUsuario/Projetos):
...
3. **Download:** Se você baixou o ZIP, descompacte-o e navegue para a pasta do projeto.

### Passo 4: Configuração e Ativação do Ambiente Virtual (Venv)

1. **Criar a Venv:** Crie o ambiente virtual (recomendamos nomeá-lo .venv):

```python3 -m venv .venv```

2. **Ativar a Venv:** Ative o ambiente virtual para isolar as dependências do sistema:

```source .venv/bin/activate```

*(Seu terminal exibirá (.venv) no prompt, indicando que o ambiente está ativo.)*

3. **Desativar a Venv:** Para sair do ambiente virtual a qualquer momento:
...
1. **Navegar até o Local:** Navegue até o diretório de sua preferência:

```cd ~/Projetos```

2. **Clonar (Git):** Clone o repositório:
...
2. **Ativar a Venv:** Ative o ambiente virtual:

```source .venv/bin/activate```

*(Seu terminal exibirá (.venv) no prompt.)*

3. **Desativar a Venv:** Para sair do ambiente virtual:
...
O Streamlit iniciará um servidor e abrirá a aplicação em seu navegador padrão, em caso de não abrir automaticamente acesse:
